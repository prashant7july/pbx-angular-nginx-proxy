...
@Component({
  ...
  templateUrl: './view-blacklist.component.html',
  styleUrls: ['./view-blacklist.component.css']
})
export class ViewBlacklistComponent implements OnInit {
  ...
  displayAllRecord() {
    this.columnDefs = [
      ...
      { field: 'blocked_for', headerName: 'Blocked For', hide: false, width: 10 },
    ...
    if (this.isFilter) {
      ...
      credentials.by_blocked_for = Number(credentials.by_blocked_for)
      
      this.blackListService.filterBlackList(credentials, Number(localStorage.getItem('id')), Number(localStorage.getItem('type'))).subscribe(data => {
      ...
      this.blackListService.getBlackList({ id: null, phonenumber: null, 'customer_id': Number(this.customerId), 'extension_id': Number(this.extensionId), 'role': Number(localStorage.getItem('type')), 'country': null }).subscribe(data => {
        this.exportData = data;
        data = this.manageUserActionBtn(data);
        this.dataSource = [];
        this.dataSource.push({ 'fields': this.columnDefs, 'data': data });
  ...
  exportToExcel(): void {
    ...
    worksheet.columns = [
      ...
      { header: 'Blocked Type', key: 'reserved_for', width: 20 },
    ...
    workbook.xlsx.writeBuffer().then(data => {
      ...
      FileSaver.saveAs(blob, 'blacklist');
    });
  }

  downloadPDF(): void {
    var doc = new jsPDF();
    var col = ["Country", "Phone Number", "Name","Blocked Type","Status"];
    var rows = [];
    this.exportData.forEach(element => {
      let strStatus = element.status;
      let strStatus1 = strStatus.replace(/<[^>]*>/g, '');
      let reserved = (element.blocked == '1') ? 'Incoming' : (element.blocked == '2') ? 'Outgoing' :'Both';
      const e11 = [element.country, element.phone_number, element.name, reserved, strStatus1];
      rows.push(e11);
    });
  ...
  manageUserActionBtn(data) {
    for (let i = 0; i < data.length; i++) {
      ...
      let finalBtn1 = '';
      finalBtn += "<span>";
      data[i]['blocked'] = data[i]['blocked_for'];
  ...
  deleteData(event) {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        this.blackListService.deleteBlackListContact({ 'id': event.id }).subscribe(data => {
          this.displayAllRecord();
        },
          err => {
            this.error = err.message;
  ...
  updateStatus(event) {
    ...
    Swal.fire({
      title: '<span style="color:#FFFFFF;">Are you sure?</span>',
      html: "<span style='color:#FFFFFF;'>You can " + blackListContactStatus + " blacklist contact </span><span style ='color:red; font-weight :bold; font-size: 1.2em'>"+event.name+"</span><span style='color:#FFFFFF;'> in future!</span>",
      type: 'warning',
      showCancelButton: true,
      background: '#000000',
      ...
      cancelButtonText: 'No, keep it',
      preConfirm: () => {
        this.blackListService.updateBlackListContactStatus('updateBlackListContactStatus', id, action).subscribe(data => {
          this.displayAllRecord();
        }, err => {
          this.error = err.message;
        });
  ...
  openDialog(id?, data?): void {    
    const dialogRef = this.dialog.open(BlacklistDialog, {
...
export class InfoBlackListDialog {
  constructor(
    public dialogRefInfo: MatDialogRef<InfoBlackListDialog>, @Inject(MAT_DIALOG_DATA) public data: BlackList,
  ) { }
...
@Component({
  selector: 'blacklist-dialog',
  templateUrl: 'blacklist-dialog.html',
})
...
export class BlacklistDialog {
  ...
  constructor(
    public dialogRef: MatDialogRef<BlacklistDialog>, @Inject(MAT_DIALOG_DATA) public data: BlackList,
  ...
  get phone() { return this.blackListForm.get('phone'); }
  get phone2() { return this.blackListForm.get('phone2'); }
  get country() { return this.blackListForm.get('country'); }
  get name() { return this.blackListForm.get('name'); }
  ...
  ngOnInit() {
    this.blackListForm.controls.country.disable();
    this.blackListForm.controls.phone.disable();
    this.blackListForm.controls.phone2.enable();
    ...
    if (this.data.id) {            
      this.blackListService.getBlackList({ id: this.data.id, phonenumber: null, 'customer_id': Number(this.data['customer_id_real']), 'extension_id': Number(this.data['extensionId']), 'role': Number(localStorage.getItem('type')), 'country': null }).subscribe(data => {
        ...
        this.blockForCall = this.blackListContactData.blocked_for ? this.blackListContactData.blocked_for : '1';
        if (this.blackListContactData.country_id != 0) {
          this.blackListForm.controls.country.enable();
          this.blackListForm.controls.phone.enable();
          this.blackListForm.controls.phone2.disable();
  ...
  submitBlackListForm() {
    ...
    if (this.blackListForm.valid) {
      this.submitted = true;
      const credentials = this.blackListForm.value;
      ...
      credentials.phone = Number(credentials.phone)
      credentials.blockFor = Number(credentials.blockFor)
      if (credentials.numberFormat && credentials.country == '') {
        this.toastr.error('Error!', countryError, { timeOut: 4000 });
        return;
      }
      if(credentials.phone){
        this.blackListService.getBlackList({ 'id': credentials.id, 'phonenumber': "'"+credentials.country_code + credentials.phone+"'", 'customer_id': Number(credentials.customer_id), 'extension_id': Number(credentials.extension_id), 'role': Number(localStorage.getItem('type')), 'country': credentials.country }).subscribe(data => {        
          if (data && data[0].lastInserted >= 1 && this.data.id != data[0].lastInserted) {
            ...
            this.blackListForm.controls['phone'].setValue('');
            credentials.phone == '';
            return;        
          }else{
              credentials.country_name = this.countryList.filter(item => item.id == credentials.country)[0]['name'];
             this.blackListService.createBlackListContact('createBlackListContact', credentials)
              .subscribe(data => {
                if (data['code'] == 200) {
        ...
        this.blackListService.getBlackList({ 'id': credentials.id, 'phonenumber': "'"+credentials.phone2+"'", 'customer_id': Number(credentials.customer_id), 'extension_id': Number(credentials.extension_id), 'role': Number(localStorage.getItem('type')), 'country': null }).subscribe(data => {
          if (data && data[0].lastInserted >= 1 && this.data.id != data[0].lastInserted && !credentials.numberFormat) {
            this.errorField = data[0].MESSAGE_TEXT;
            this.toastr.error('Error!', this.errorField, { timeOut: 4000 });
            this.blackListForm.controls['phone2'].setValue('');
            credentials.phone2 == '';
            return;
          }else {
            this.blackListService.createBlackListContact('createBlackListContact', credentials)
  ...
  public findInvalidControls() {
    const invalid = [];
    const controls = this.blackListForm.controls;
