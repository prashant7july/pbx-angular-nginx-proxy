...
@Component({
  selector: 'app-customers-call-detail',
  templateUrl: './customers-call-detail.component.html',
  styleUrls: ['./customers-call-detail.component.css'],
  providers: [CheckBoxSelectionService] 
})
export class CustomersCallDetailComponent implements OnInit {

  //------------------------------  Add Customers calls Details By. (Nagender Pratap Chauhan 18-08-2021) ------------------------------------------------------------//
  ...
  displayAllRecord() {
    this.columnDefs = [
      ...
      { field: 'sip_minutes', headerName: 'SIP Minutes', hide: false, width: 150 },
      { field: 'total_charges', headerName: 'Total Charges', hide: false, width: 150 },
      { field: '', headerName: 'IM Count', hide: false, width: 150 },
      ...
      if (this.isFilter) {
        ...
        this.reportService.getCustomersCallDetailsByFilters(credentials,role,ResellerID).subscribe(pagedData => {
        ...
        this.reportService.getCustomersCallDetails(role,ResellerID).subscribe(pagedData => {
          this.exportData = pagedData;
           pagedData = this.manageUserActionBtn(pagedData);
          this.dataSource = [];
          this.dataSource.push({ 'fields': this.columnDefs, 'data': pagedData });
  ...
  exportToExcel(): void {
    ...
    worksheet.columns = [
      ...
      { header: 'Outgoing Minutes', key: 'Outgoing_Minutes', width: 30 },
      { header: 'Incoming Minutes', key: 'Incoming_Minutes', width: 30 },
      { header: 'SIP Minutes', key: 'SIP_Minutes', width: 30 },
      { header: 'Outgoing Charges', key: 'Outgoing_Charges', width: 30 },
      { header: 'Incoming Charges', key: 'Incoming_Charges', width: 30 },
    ...
    workbook.xlsx.writeBuffer().then(data => {
      ...
      FileSaver.saveAs(blob, 'customer-call-details');
    });
  }
  
  downloadPDF(): void {
    var doc = new jsPDF();
    var col = ["Customer", "Outgoing Minutes", "Incoming Minutes","SIP Minutes", "Outgoing Charges",  "Incoming Charges", "Total Charges"];
    var rows = [];
    this.exportData.forEach(element => {
      const e11 = [element.customer_name, element.outgoing_minutes,element.incoming_minutes, element.sip_minutes, element.outgoing_charge,  element.incoming_charge, element.total_charges];
      rows.push(e11);
    });
    ...
    doc.save('customer-call-details.pdf');
  }
