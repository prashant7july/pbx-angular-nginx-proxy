...
@Component({
  selector: 'app-date-wise-call-charges',
  templateUrl: './date-wise-call-charges.component.html',
  styleUrls: ['./date-wise-call-charges.component.css'],
  providers: [CheckBoxSelectionService] 
})
export class DateWiseCallChargesComponent implements OnInit {

 //------------------------------  Add Date wise calls Charges By. (Nagender Pratap Chauhan 09-08-2021) ------------------------------------------------------------//
    ...
    { field: 'action', headerName: 'Action', hide: false, width: 150 },
     ...
     if (this.isFilter) {
      ...
      let ResellerID = localStorage.getItem('id');
       ...
       this.reportService.getCallChargesDateWiseByFilters(credentials,role,ResellerID).subscribe(pagedData => {
      ...
      let role = localStorage.getItem('type');
      let ResellerID = localStorage.getItem('id');
       this.reportService.getCallChargesDateWise(role,ResellerID).subscribe(pagedData => {
         this.exportData = pagedData;
          pagedData = this.manageUserActionBtn(pagedData);
         this.dataSource = [];
         this.dataSource.push({ 'fields': this.columnDefs, 'data': pagedData });
 ...
 exportToExcel(): void {
  ...
  workbook.xlsx.writeBuffer().then(data => {
    ...
    FileSaver.saveAs(blob, 'date-wise-call-charges');
  });
}

downloadPDF(): void {
  var doc = new jsPDF();
  var col = ["Date", "Company", "Outgoing Minutes", "Outgoing Charges", "Incoming Minutes", "Incoming Charges", "Total Minutes", "Total Charges"];

  var rows = [];
  this.exportData.forEach(element => {
    const e11 = [element.startTime, element.customer_name, element.outgoing_minutes, element.outgoing_charge, element.incoming_minutes, element.incoming_charge, element.total_minutes, element.total_charges];
    rows.push(e11);
  });
  ...
  doc.save('date-wise-call-charges.pdf');
}
