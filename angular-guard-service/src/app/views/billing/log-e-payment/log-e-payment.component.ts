...
@Component({
  selector: 'app-log-e-payment',
  templateUrl: './log-e-payment.component.html',
  styleUrls: ['./log-e-payment.component.css']
})
export class LogEPaymentComponent implements OnInit {
  ...
  displayAllRecord() {
    this.columnDefs = [
      ...
      { field: 'company', headerName: 'Company', hide: false, width: 8 },
      { field: 'invoice_number', headerName: 'Invoice Number', hide: false, width: 8 },
      { field: 'paymentmethod', headerName: 'Payment Method', hide: false, width: 10 },
      { field: 'currency', headerName: 'Currency', hide: false, width: 8 },
    ...
    if (this.isFilter) {
      ...
      credentials.by_invoice_number = Number(this.filterForm.value.by_invoice_number);      
      this.billingService.filterEBillingInfo(credentials,role,ResellerID).subscribe(pagedData => {
      ...
      this.billingService.getEBillingInfo({role:role,ResellerID:ResellerID}).subscribe(pagedData => {
        this.exportData = pagedData;
        this.dataSource = [];
        this.dataSource.push({ 'fields': this.columnDefs, 'data': pagedData });
      });
  ...
  exportToExcel(): void {
    ...
    for (let i = 0; i < this.exportData.length; i++) {
      worksheet.addRow({
        ...
        invoice_number: this.exportData[i].invoice_number,
        company: this.exportData[i].company,
        paymentmethod: this.exportData[i].paymentmethod,
        currency: this.exportData[i].currency,
        status: this.exportData[i].status,
  ...
  downloadPDF(): void {
    ...
    var doc = new jsPDF();
    var col = ["Date", "Invoice Number", "Company",  "Payment Method", "Currency", "Status","Amount"];
    var rows = [];
    this.exportData.forEach(element => {
      const e11 = [element.date, element.invoice_number, element.company,element.paymentmethod, element.currency, element.status, element.amountDisplay];
      rows.push(e11);
    });
  ...
  showInfo() {
    const dialogRefInfo = this.dialog.open(InfoLogEPaymentDialog, {
...
@Component({
  selector: 'infoLogEPayment-dialog',
  templateUrl: 'infoLogEPayment-dialog.html',
})

export class InfoLogEPaymentDialog {
  constructor(
    public dialogRefInfo: MatDialogRef<InfoLogEPaymentDialog>, @Inject(MAT_DIALOG_DATA) public data: '',
  ) { }
