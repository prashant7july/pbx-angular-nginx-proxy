...
import { RestrictionService } from '../../Access-Restriction/restriction.service';
...
@Component({
  selector: 'app-support-ip-restriction',
  templateUrl: './support-ip-restriction.component.html',
  styleUrls: ['./support-ip-restriction.component.css']
})
export class SupportIpRestrictionComponent implements OnInit{
  columnDefs: any;
    ...
    if (userType == UserTypeConstants.ADMIN) {
      this.userService.getCustomerCompany(productId).subscribe(datas => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          this.filter =this.companyList.slice();
      ...
      this.userService.getCustomerCompany(productId).subscribe(datas => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // this.companyListTwo.push({ id: data[i].id, name: data[i].company_name + '(Ref. Code: ' + data[i].id + ')' });
          this.droplist.push({ id: data[i].id, name: data[i].company_name });
          this.filterProducthtml = this.ProducthtmlFilter = this.droplist.slice();
        }
      }, err => {
  ...
  public displayAllRecord() {
    ...
    if (this.isFilter) {
      ...
      this.RestrictionService.getViewAccessFilter(credentials,'','').subscribe(pagedData => {
        this.isFilter = false;
      ...
      this.RestrictionService.viewAccessRestriction(userId).subscribe(pagedData => {    
        userId = '';
        this.exportData = pagedData;
        pagedData = this.manageUserActionBtn(pagedData);
        this.dataSource = [];
