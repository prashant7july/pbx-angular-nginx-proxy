...
@Component({
  selector: 'app-conference-cdr',
  templateUrl: './conference-cdr.component.html',
  styleUrls: ['./conference-cdr.component.css']
})
export class ConferenceCDRComponent implements OnInit {
  ...
  Accountremoved(event){
    ...
    this.cdrService.getTerminateCause().subscribe(data => {
      for (let i = 0; i < data.length; i++) {
        this.terminatecause.push({ id: data[i].digit.toString(), name: data[i].digit + '-' + data[i].description });
      }
  ...
  displayAllRecord() {
    ...
    this.columnDefs = [
      ...
      { field: 'action', headerName: 'Action',width : 50, hide: false},
      { field: 'conf_name', headerName: 'Conference Name',width : 170, hide: false},
      { field: 'instance_id', headerName: 'Instance ID' ,width : 170, hide: true},
      // { field: 'caller', headerName: 'Caller' ,width : 150,hide: true},
      // { field: 'callee', headerName: 'Callee', width : 150, hide: true} ,
      // { field: 'bridge_time', headerName: 'Bridge Time' ,width : 150, hide: false},
      { field: 'startTime', headerName: 'Start Time',width : 170, hide: false},
      { field: 'endTime', headerName: 'End Time' ,width : 170, hide: false},
      { field: 'total', headerName: 'Members',hide: false },
    ...
    if (this.isFilter) {
      ...
      this.cdrService.getConferenceByFilter(credentials).subscribe(pagedData => {
      ...
      this.cdrService.getConferenceData(user_id).subscribe(pagedData => {
        this.exportData = pagedData;        
        pagedData = this.manageUserActionBtn(pagedData);
        this.dataSource = [];
        this.dataSource.push({ 'fields': this.columnDefs, 'data': pagedData });
  ...
  manageUserActionBtn(data) {
    ...
    //   }
      ...
      actionBtn += "<i class='fa fa-list list-button' style='cursor:pointer; display: inline' data-action-type='list_cdr' title='List CDR'></i>";
      // }
  ...
  openLocationDialog(id?): void {
    const dialogRef = this.dialog.open(ListDialog, { width: '90%', disableClose: true, data: id });
  ...
  showInfo() {
    const dialogRefInfo = this.dialog.open(ConferenceCdrDialog, {
...
export class ListDialog {
  ...
  constructor(
    public dialogRefInfo: MatDialogRef<ListDialog>, @Inject(MAT_DIALOG_DATA) public data: '',
    private router: Router,
    private fb: FormBuilder,
    public commonService: CommonService,
    private excelService: ExcelService,
  ...
  ngOnInit() {
    let user_id = localStorage.getItem('id');
     this.instance = this.data['instance_id'];
    
    this.cdrService.getConferenceCdr(user_id,this.instance).subscribe(pagedData => {
      this.exportData = pagedData;
      pagedData = this.manageUserActionBtn(pagedData);
      this.dataSource = [];
      this.dataSource.push({ 'fields': this.columnDefs, 'data': pagedData });
  ...
  advanceExportToExcel(){
    const dialogRefInfo = this.dialog.open(ConferenceCdrAdvanceExcelDialog, {
      width: '60%', disableClose: true, autoFocus: false,
      data: {
        customerId: localStorage.getItem('type') == '1' ? localStorage.getItem('id') : 0,exportData : this.exportData, instance_id : this.instance
      }
    });
    dialogRefInfo.keydownEvents().subscribe(e => {
      if (e.keyCode == 27) {
...
@Component({
  selector: 'ConferenceCdrInfo-dialog',
  templateUrl: 'ConferenceCdrInfo-dialog.html',
})

export class ConferenceCdrDialog {
  constructor(
    public dialogRefInfo: MatDialogRef<ConferenceCdrDialog>, @Inject(MAT_DIALOG_DATA) public data: '',
  ) { }
...
export class ConferenceCdrAdvanceExcelDialog {
  columnList: any[] = [];
  selectedColumn : any;
  selectedDate : any ;
  public mode ;
  ...
  constructor(
    public dialogRefInfo: MatDialogRef<ConferenceCdrAdvanceExcelDialog>,
  ...
  submitDialog():void {
    ...
    if (this.selectedColumn == undefined || this.selectedDate == undefined) {
      ...
      credentials['instance_id'] = this.instance;
      this.cdrService.getExportExcelData(credentials).subscribe(pagedData => {
        this.exportData = pagedData;
        this.downloadCSV();
        this.dialogRefInfo.close();
      });
